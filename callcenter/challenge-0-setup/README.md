...
If a Foundry resource and model deployment have already been provided, no Azure login is needed to run Challenges 1–4. Copy [`.env.template`](./.env.template) to `callcenter/.env` and set `FOUNDRY_ENDPOINT`, `MODEL_DEPLOYMENT_NAME`, and `API_KEY`. Set the Application Insights connection string as well to complete Challenge 2. Skip infrastructure deployment below.
