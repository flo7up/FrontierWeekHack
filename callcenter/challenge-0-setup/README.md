...
If the facilitator already provided a configured `callcenter/.env`, keep it and skip to step 4.

1. In VS Code's file list, open `callcenter/challenge-0-setup/workshop.env.template` ([view template](./workshop.env.template)).
2. Choose **File > Save As**. Save a new file named exactly `.env` in the `callcenter` folder, one level above `challenge-0-setup`. Do not name it `.env.txt`.
...
The final location is **`callcenter/.env`**, not `callcenter/challenge-0-setup/.env`. Keep the public template unchanged.
