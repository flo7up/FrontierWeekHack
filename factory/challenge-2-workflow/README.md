...
```bash
python factory/challenge-2-workflow/workflow.py
```

### Local Windows PowerShell

```powershell
.\.venv\Scripts\python.exe factory/challenge-2-workflow/workflow.py
```

### Local macOS or Linux

```bash
./.venv/bin/python factory/challenge-2-workflow/workflow.py
```
...
Open [workflow.py](./workflow.py) in `factory/challenge-2-workflow`. Find `run_fault_diagnosis()` and the `instructions` text containing `Use at most 120 words.` Replace that sentence with:
