...
Choose **Terminal > New Terminal** in VS Code. Run every workshop command from this main folder, not from inside `factory`. Copy one line at a time and press Enter; wait for it to finish before the next line.
...
If `factory/.env` already exists, do not overwrite it. Open that file instead and check that it contains the current private key. If it is already configured, skip to step 4.

1. In the Codespaces or VS Code file list, expand the `factory` folder and find `.env.template` directly inside it.
...
The final location is **`factory/.env`**, not `factory/challenge-0-setup/.env`. Enter the key only after renaming: never put a real key in a tracked template or commit the local rename/deletion. If `.env.template` is missing, use the [downloadable template](./workshop.env.template) and name the downloaded file `factory/.env`.
...
In Codespaces, create or edit `.env` in the browser editor. A file on your laptop is not transferred automatically. If the facilitator provides a ready-made `.env`, upload it into `factory` in the Codespace instead of renaming the template; do not overwrite an existing configuration.
...
```powershell
.\.venv\Scripts\python.exe smoke_test.py factory
```

### Local macOS or Linux

```bash
./.venv/bin/python smoke_test.py factory
```
