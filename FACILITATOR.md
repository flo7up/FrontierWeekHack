...
## Optional DevUI Bonus

Use [Bonus: Chat and Inspect Agent Traces](./docs/bonus-devui.md) for participants who have completed the core flow. Budget 20-30 minutes instead of one optional experiment or after the scheduled session; do not make it another required setup hurdle.

- Install the pinned `bonus-devui/requirements.txt` into `.venv-devui`, not the core environment. The bonus is a development sample and its API changes between versions.
- The launcher is `bonus-devui/devui.py <scenario>`. It registers one framework agent with the existing scenario tool, loads the scenario's model settings, binds to localhost, and enables local OpenTelemetry tracing.
- In Codespaces, forward port 8080 privately and open it through the Ports tab. The configuration labels this port but does not install or start DevUI automatically. Existing Codespaces can forward it manually without a rebuild.
- Explain the two credentials: the Foundry key stays in the private scenario `.env`; the autogenerated DevUI token unlocks the local UI. Do not project either token during sign-in.
- The bonus intentionally captures fictional prompts, tool inputs/results, and replies in the DevUI session. It does not configure an external trace exporter. Never use production data or expose the DevUI port publicly.
- Use one or two small record-level questions, not all-record batch prompts. The UI shares the same model quota and remaining credits as the core exercises.
- Local Windows browser preflight verified an authenticated Factory tool call with `grok-4.6`, a streamed response, and a visible `execute_tool check_thresholds` span with arguments/results. All three scenario agents/tools also have offline checks. A fresh Codespaces/browser-forwarding test and full live checks for other scenarios are still required before presenting them as validated for your audience.

Run bonus offline checks after installation:

```bash
./.venv-devui/bin/python -m unittest discover -s bonus-devui -p 'test_*.py' -v
```

On Windows use `.\.venv-devui\Scripts\python.exe` instead. For a Codespaces preflight, confirm login, streamed text, a visible tool-result span, and private port visibility. If the browser route fails, keep the bonus instructor-led instead of weakening port security.
